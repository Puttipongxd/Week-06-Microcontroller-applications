...
ทำให้โค้ดอ่านง่าย ไม่ยาวเกินไปในไฟล์เดียว
ช่วยแบ่งตามหน้าที่ (modularity) เช่น sensor, display, led
ลดความซับซ้อน → แก้บั๊ก/เพิ่มฟีเจอร์ได้เฉพาะส่วน โดยไม่กระทบไฟล์อื่น
เพิ่มความสามารถในการ re-use code (เรียกใช้ซ้ำในโปรเจกต์อื่นได้)
2. **CMakeLists.txt Management**: การเพิ่มไฟล์ source ใหม่ต้องแก้ไขอะไรบ้าง?
ต้องเพิ่มชื่อไฟล์ .c (หรือ .cpp) ใหม่เข้าไปใน CMakeLists.txt ของ component (เช่น main/CMakeLists.txt)
3. **Header Files**: บทบาทของไฟล์ .h คืออะไร และทำไมต้องมี?
ประกาศ function, struct, macro ให้ไฟล์อื่นเรียกใช้ได้
เป็น “สัญญา (contract)” ระหว่าง module → ไม่ต้องรู้ implementation แต่เรียกใช้ function ได้
ช่วยให้ compile เร็วขึ้น เพราะ compiler แยกส่วนการแปล (compile) ของแต่ละไฟล์
4. **Include Directories**: เหตุใด CMakeLists.txt ต้องระบุ INCLUDE_DIRS?
เพื่อบอก compiler ว่าจะไปหาไฟล์ .h ได้จากที่ไหน
ถ้าไม่ใส่ → เวลา #include "sensor.h" จะหาไฟล์ไม่เจอ
5. **Git Ignore**: ไฟล์ .gitignore ช่วยอะไรในการจัดการ ESP32 project?
กันไม่ให้ไฟล์ชั่วคราว / build files ขนาดใหญ่ (build/, *.bin, *.elf) ถูก push เข้า repo
กันไฟล์ config ส่วนตัว (เช่น .vscode/, .idea/) ที่ไม่เกี่ยวกับ project logic
ทำให้ repo สะอาด, clone เร็ว และทุกคน build ได้เองจาก source + CMake
6. **Task Management**: การใช้ FreeRTOS task ในโมดูล LED ช่วยอะไร?
ทำให้ LED ทำงาน ขนาน (concurrent) กับงานอื่น เช่น อ่าน sensor หรือสื่อสาร UART
ไม่ block งานหลัก → เช่น LED กระพริบเองใน loop แยก ไม่ไปขัด sensor reading
สามารถปรับ priority, delay, scheduling ของ task ได้ง่าย
7. **Code Organization**: ข้อดีของการแยกโมดูล sensor, display, led เป็นไฟล์แยกคืออะไร?
Modularity → แต่ละโมดูลทำหน้าที่เฉพาะ เช่น sensor อ่านค่า, display แสดงผล, led กระพริบ
Maintainability → ง่ายต่อการแก้ไข/เพิ่มฟีเจอร์ (แก้ sensor ไม่กระทบ display)
Reusability → ยกไปใช้ในโปรเจกต์อื่นได้ทันที
Collaboration → ทีมงานแต่ละคนทำงานบน module ของตัวเองได้ ไม่ชนกัน
