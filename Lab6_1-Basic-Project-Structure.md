...
Isolation (แยกสภาพแวดล้อม) → ไม่ต้องกลัว dependency conflict กับเครื่อง host เช่น Python เวอร์ชัน, Toolchain, Path
2. **Build Process**: อธิบายขั้นตอนการ build ของ ESP-IDF ใน Docker container ตั้งแต่ source code จนได้ binary
Source Code + CMakeLists.txt → อยู่ใน project folder บน host และถูก mount เข้า container
3. **CMake Files**: บทบาทของไฟล์ CMakeLists.txt แต่ละไฟล์คืออะไร และทำงานอย่างไรใน Docker environment?
Top-level CMakeLists.txt (root ของ project) → บอกว่าโปรเจกต์ชื่ออะไร, ใช้ ESP-IDF, include component ไหน
4. **Git Ignore**: ไฟล์ .gitignore มีความสำคัญอย่างไรสำหรับ ESP32 project development?
ป้องกันไม่ให้ไฟล์ชั่วคราวหรือไฟล์ build ที่ไม่จำเป็นถูก push เข้า git
5. **Container Persistence**: ข้อมูลใดบ้างที่จะหายไปเมื่อ restart container และข้อมูลใดที่จะอยู่ต่อ?
หายไปเมื่อ restart container (ถ้าไม่ได้ mount volume):
build artifacts (build/)
sdkconfig (ถ้าไม่ได้ map)
local Python packages ที่ลงเพิ่มใน container
6. **Development Workflow**: เปรียบเทียบ workflow การพัฒนาระหว่างการใช้ Docker กับการทำงานบน native system
Native System
Setup: ติดตั้ง ESP-IDF, Python, Toolchain บนเครื่อง
Workflow: idf.py build / flash / monitor ใช้งานตรง ๆ
Pros: compile เร็วกว่า, integration ดีกับ IDE
Cons: dependency conflict ง่าย, ต้อง setup environment เอง, ย้ายเครื่องลำบาก
Docker
Setup: ดึง image, run container, mount project
Workflow: run command ผ่าน docker exec หรือ idf.py ใน container
Pros: portable, reproducible, ง่ายต่อการ onboard ทีมใหม่
Cons: compile ช้ากว่านิดหน่อย, integration debug อาจยุ่งกว่า, ต้องจัดการ volume mount
